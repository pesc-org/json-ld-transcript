#! /usr/bin/bash

# Phil's script to automatically generate ttl files of value vocabs based on CSV tabular vocabulary definitions.
# in testing, script not yet published.

DIR='Vocabs/'             # path to directory with csv tabular vocab definitions files in
NS='TAP+SHACL/namespaces.csv' # path to namespaces csv


for file in AcademicAwardLevels AcademicProgramTypes AcademicSummaryTypes AccreditationBodyCodes AgencyCodes ConditionsMetCodes CourseLevels CreditBasis CreditLevels CreditUnits DelinquencyCodes DocumentCompleteCodes DocumentOfficialCodes DocumentProcessCodes DocumentTypeCodes GenderCodes GradeStatusCodes HonorsLevels InstructionSiteCodes LanguageProficiencyCodes LanguageUsageCodes NameCodes ResidencyStatusCodes SchoolLevels SchoolOverrideCodes SessionTypes StudentLevelCodes SupplementalGradeCodes TestScoreMethods TransmissionTypes
do
    echo $file
    /home/phil/bin/tvd convert -ns $NS  -of turtle $DIR$file.csv $DIR$file.ttl
    /home/phil/bin/tvd convert -ns $NS  -of xml $DIR$file.csv $DIR$file.xml
    /home/phil/bin/pylode -p pescvoc -o  $DIR$file.html $DIR$file.ttl
done
